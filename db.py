...
def update_progress(user_id, vocab_id, correct: bool):
    ...
    既存データをSupabaseへ毎回問い合わせるため、連続で答えるときは
    record_answer(progress_cacheを使う版)の方が高速。"""
    client = get_client()
    existing = (
    ...
    return _write_progress(client, user_id, vocab_id, correct, row)


def record_answer(user_id, vocab_id, correct: bool, progress_cache):
    """
    チャレンジモード用の高速版。
    既存の状態をSupabaseへ問い合わせず、あらかじめ読み込んでおいた
    progress_cache(dict: vocab_id -> row)から読む。
    Supabaseへの通信は書き込み(upsert)の1回だけで済むため、
    クリックのたびに複数回通信していたときより反応が速くなる。
    progress_cacheはその場で更新されるので、呼び出し側は使い回せる。
    """
    client = get_client()
    row = progress_cache.get(vocab_id)
    new_row, newly_learned = _write_progress(client, user_id, vocab_id, correct, row)
    progress_cache[vocab_id] = new_row
    return newly_learned


def _write_progress(client, user_id, vocab_id, correct, row):
    if row:
        box = row["box"]
    ...
    newly_learned = correct and correct_count == 0
    ...
    client.table(TABLE_NAME).upsert(new_row, on_conflict="user_id,vocab_id").execute()
    return new_row, newly_learned


def compute_level_summary(progress, vocab_by_level):
    """
    get_all_level_summaryの中身を、Supabaseへの問い合わせなしで
    (すでに取得済みのprogress辞書から)その場で計算する版。
    """
    levels = {}
    total_correct = 0
    total_learned = 0
...
def get_all_level_summary(user_id, vocab_by_level):
    ...
    return compute_level_summary(progress, vocab_by_level)
