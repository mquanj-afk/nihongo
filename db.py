...
def get_all_level_summary(user_id, vocab_by_level):
    ...
    ゲーミフィケーション表示用の合計正解数・合計習得語数をまとめて返す。

    percentは「6回連続正解して完全に覚えた単語の割合」ではなく、
    box(1〜6)の合計を最大値に対する割合として計算する。
    これにより、正解するたびに(6回貯めなくても)少しずつ%が動き、
    「進捗が反映されない」と感じにくくなる。
    """
    progress = get_all_progress(user_id)
    levels = {}
    ...
    for level, vocab_ids in vocab_by_level.items():
        ...
        total = len(vocab_ids)
        total_words += total
        max_possible = total * MAX_BOX
        percent = round(box_sum / max_possible * 100) if max_possible else 0
        levels[level] = {"mastered": mastered, "total": total, "percent": percent}
