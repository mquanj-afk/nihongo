...
with st.sidebar:
    ...
    st.divider()
    # 全レベル分をまとめて1回で取得(通信を節約)
    vocab_by_level = {lv: [v["id"] for v in get_vocab_by_level(lv)] for lv in [1, 2, 3, 4, 5]}
    summary = db.get_all_level_summary(user_id, vocab_by_level)

    st.header("🏆 あなたの称号")
    rank_title = db.get_rank_title(summary["total_correct"])
    st.markdown(f"### {rank_title}")
    st.caption(f"通算正解数: {summary['total_correct']} 問 / 習得済み単語: {summary['total_mastered']} / {summary['total_words']}")

    st.divider()
    st.header("📊 レベル別 習得率")
    for lv in [1, 2, 3, 4, 5]:
        lv_stat = summary["levels"][lv]
        label = LEVEL_NAMES[lv]
        marker = " ✅" if lv_stat["percent"] == 100 else ""
        st.caption(f"{label}{marker}")
        st.progress(lv_stat["percent"] / 100, text=f"{lv_stat['percent']}%({lv_stat['mastered']}/{lv_stat['total']}語)")
...
def submit_answer(choice, timed_out=False):
    ...
    if is_correct:
        ...
        level_vocab_ids = [v["id"] for v in get_vocab_by_level(level)]
        stats = db.get_level_stats(user_id, level_vocab_ids)
        if stats["mastered"] == len(level_vocab_ids):
            if level not in st.session_state.celebrated_levels:
...
streak_display = "🔥" * min(streak, 5) if streak > 0 else "―"
col3.metric("連続正解", f"{streak}問", streak_display if streak > 0 else None)
