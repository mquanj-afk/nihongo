...
with st.sidebar:
    ...
    st.caption(f"テストモード:1回{TEST_QUESTIONS}問(このレベルの単語からランダム出題)")
...
def start_test():
    ...
    # 毎回シャッフルしてランダムに出題(語彙数が問題数以上あれば重複なし)
    queue = db.build_test_queue(vocab_ids, TEST_QUESTIONS)
...
# ---------------- テスト開始処理 ----------------
if not st.session_state.test_queue and not st.session_state.test_finished:
    start_test()
...
st.caption(
    f"第 {st.session_state.test_index + 1}/{TEST_QUESTIONS} 問 ・ "
    f"正解 {st.session_state.session_correct}/{st.session_state.session_total}{streak_display}"
)
...
if not st.session_state.answered:
    ...
    is_last = st.session_state.test_index + 1 >= TEST_QUESTIONS
