...
BGM_BYTES = base64.b64decode(BGM_LOOP_B64)
...
with st.sidebar:
        ...
        with tab_login:
            ...
            if st.button("ログイン", use_container_width=True, type="primary"):
                try:
                    ...
                    db.upsert_profile(result.user.id, result.user.email.split("@")[0])
                    st.rerun()
    ...
    # ランキングも答えるたびに更新すれば十分(毎秒の自動更新では呼ばない)
    if "leaderboard" not in st.session_state:
        st.session_state.leaderboard = db.get_leaderboard(limit=10)

    st.header("🏆 正解数ランキング")
    medals = ["🥇", "🥈", "🥉"]
    if not st.session_state.leaderboard:
        st.caption("まだランキングデータがありません。")
    for i, entry in enumerate(st.session_state.leaderboard):
        rank_icon = medals[i] if i < len(medals) else f"{i + 1}."
        you_mark = " (あなた)" if entry["user_id"] == user_id else ""
        st.markdown(f"{rank_icon} **{entry['name']}**{you_mark} ー {entry['total_correct']}問")
...
def start_challenge():
    ...
    # 進捗データを最初に1回だけ読み込み、以降はこのキャッシュを使い回す
    # (クリックのたびに読み込み直すと通信が増えて反応が遅くなるため)
    st.session_state.progress_cache = db.get_all_progress(user_id)
    # ランキングもチャレンジ開始時に1回更新しておく(他の人の最新結果を反映)
    st.session_state.leaderboard = db.get_leaderboard(limit=10)
    load_new_question()
...
def submit_answer(choice, timed_out=False):
    ...
    # Supabaseへの通信は書き込み1回だけ(既存データはキャッシュから読む)
    newly_learned = db.record_answer(user_id, vocab_item["id"], is_correct, st.session_state.progress_cache)

    st.session_state.trigger_balloons = False
    st.session_state.play_correct_sound = False
    st.session_state.play_wrong_sound = False

    # レベル別習得率は、更新済みのキャッシュから通信なしでその場で計算する
    vocab_by_level = {lv: [v["id"] for v in get_vocab_by_level(lv)] for lv in ALL_LEVELS}
    st.session_state.level_summary = db.compute_level_summary(st.session_state.progress_cache, vocab_by_level)
    ...
    if is_correct:
        ...
        # ランキングは自分の分だけその場で加算する(他の人の分は
        # 次にチャレンジを始めたときにまとめて最新化する)
        for entry in st.session_state.leaderboard:
            if entry["user_id"] == user_id:
                entry["total_correct"] += 1
                st.session_state.leaderboard.sort(key=lambda e: e["total_correct"], reverse=True)
                break

        new_difficulty = current_difficulty_level(st.session_state.score)
        if new_difficulty > old_difficulty:
            st.session_state.level_up_notice = new_difficulty
            st.session_state.trigger_balloons = True

        level_percent = st.session_state.level_summary["levels"][old_difficulty]["percent"]
        if level_percent == 100 and old_difficulty not in st.session_state.celebrated_levels:
            st.session_state.celebrated_levels.add(old_difficulty)
            st.session_state.trigger_balloons = True
    else:
...
def play_bgm():
    """
    st.audio はバイト列のハッシュをもとにStreamlit側でキャッシュされるため、
    毎回(自動更新のたびに)同じ約280KBのHTMLをまるごと送り直していた
    以前のcomponents.html方式より、通信量が大幅に少なくて済む。
    これによりクリックの反応の鈍さも改善されるはず。
    """
    st.audio(BGM_BYTES, format="audio/wav", autoplay=True, loop=True)
...
if st.session_state.get("level_up_notice"):
    new_lv = st.session_state.level_up_notice
    st.toast(f"🆙 LEVEL UP! 難易度が上がった:{LEVEL_NAMES[new_lv]}", icon="🎮")
    st.session_state.level_up_notice = None
